<template>
  <div>
    <div style="text-align: center; user-select: none">
      <span class="gray-title">请求批复</span>
    </div>
    <!-- 空状态显示区 -->
    <el-empty
      description="暂无请求"
      v-if="!isFind"
      :image-size="200"
      style="margin-top: 5rem; user-select: none"
    ></el-empty>
    <!-- 列表显示区域 -->
    <el-table
      class="book-table"
      :data="borrowRequestList"
      v-if="isFind"
      v-loading="loading"
      style="width: 100%"
    >
...
export default {
  ...
  methods: {
    async getBorrowRequestList() {
      const res = await this.$http.get(`/api/adminGetBorrowList`);
    ...
    async agreeWith(row) {
      ...
      const res = await this.$http.post("/api/adminAgreeBorrow", submit);
      if (res.status === 200) {
        this.$message.info({
          message: "操作成功",
          duration: 500,
