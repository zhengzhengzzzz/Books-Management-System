<template>
  <div>
    <div style="text-align: center">
      <!-- <span class="gray-title">B M 阅 览 室</span> -->
    </div>
    <!-- 用户查询区 -->
    ...
    <!-- 结果显示区 -->
    <el-table class="book-table" :data="currentPageBookList" v-if="isFind" border stripe v-loading="loading"
      style="width: 100%">
    ...
    <el-pagination @current-change="handleCurrentChange" :current-page.sync="currentPage" :page-size="pageSize"
      background layout="prev, pager, next" :total="totalItems">
    </el-pagination>
  </div>
</template>
  ...
  methods: {
    async findBook() {
      ...
      const res = await this.$http.get(
        this.baseUrl + `/findBook?name=${submit}`
      );
    ...
    async showBookInfo(row) {
      const res = await this.$http.get(
        this.baseUrl + `/getBookInfo?book_id=${row.book_id}`
      );
      if (
        res.data.book_info === null ||
        res.data.book_info.trim().length === 0
      ) {
    ...
    async borrowBook(row) {
      this.$confirm(
        `确定要借阅这本 ${row.author} 的 ${row.book_name} 吗？`,
        "提示",
        {
    ...
    handleCurrentChange(newPage) {
      ...
      // 根据索引切割原始图书列表，获取当前页的图书列表
      const newBookList = this.bookList.slice(startIndex, endIndex);

      // 更新当前页的图书列表
      this.currentPageBookList = newBookList;
    }
  },
};
</script>
